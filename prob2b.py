...
def computeCost(X,y,theta,reg, indices):
    ...
    h1 = tf.maximum(tf.cast(0, dtype=tf.float64), z1)
    z2 = tf.add(tf.matmul(h1, W2), b2)
    h2 = tf.maximum(tf.cast(0, dtype = tf.float64), z2)
    f = tf.add(tf.matmul(h2, W3), b3)
    #Softmax Calculation:
    ...
    reg_1 = (tf.reduce_sum(tf.square(W)) + tf.reduce_sum(tf.square(W2)) + tf.reduce_sum(tf.square(W3)))
...
def computeGrad(X,y,theta,reg, indices): # returns nabla
    ...
    dh2 = tf.matmul(b_p, tf.transpose(W3))
    dh2 = tf.where(tf.greater(z2,  0), dh2, tf.zeros(tf.shape(dh2), dtype=tf.float64))
    dh1 = tf.matmul(dh2, tf.transpose(W2))
    dh1 = tf.where(tf.greater(z1,  0), dh1, tf.zeros(tf.shape(dh1), dtype=tf.float64))
	# WRITEME: write your code here to complete the routine
    dW = tf.add(tf.matmul(tf.transpose(X), dh1), tf.multiply(reg, W))
    db = tf.reduce_sum(dh1, axis=0)
    dW2 = tf.add(tf.matmul(tf.transpose(h1), dh2), tf.multiply(reg, W2))
    db2 = tf.reduce_sum(dh2, axis=0)
    dW3 = tf.add(tf.matmul(tf.transpose(h2), b_p), tf.multiply(reg, W3))
    db3 = tf.reduce_sum(b_p, axis=0)
    nabla1 = tf.tuple([dW,db,dW2,db2,dW3,db3])
    return (nabla1)
...
def predict(X,theta):
    ...
    z2 = tf.add(tf.matmul(h1, W2), b2)
    h2 = tf.maximum(tf.cast(0, dtype = tf.float64), z2)
    scores = tf.add(tf.matmul(h2, W3), b3)
    exp = tf.exp(scores)
    probs = tf.divide(exp, tf.reduce_sum(exp, axis = 1, keepdims = True))
    return (scores,probs)
...
b2 = tf.Variable(tf.zeros([h2], dtype=tf.float64), dtype = tf.float64, name = "b2")
W3 = 0.01 * np.random.randn(h2,K)
W3 = tf.Variable(W3, dtype=tf.float64, name = "W3")
b3 = tf.Variable(tf.zeros([K], dtype=tf.float64), dtype = tf.float64, name = "b3")
theta = (W,b,W2,b2,W3,b3)
...
with tf.Session() as sess:
    ...
    for i in range(n_e):
        ...
        #b_t_1 = tf.convert_to_tensor(b_t1)
            ...
            W_t3 = tf.subtract(W3, tf.multiply(step_size, grand_new2[4]))
            b_t3 = tf.subtract(b3, tf.multiply(step_size, grand_new2[5]))
        #b_t_2 = tf.convert_to_tensor(b_t2)
            ...
            sess.run(tf.assign(W3, W_t3))
            sess.run(tf.assign(b3, b_t3))
